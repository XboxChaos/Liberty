...
namespace Liberty.SaveManager.Games.Halo3
{
    ...
    {
        /// <summary>
        /// Creates a new ObjectList and reads it from a EndianReader.
        /// </summary>
        /// <param name="reader">The EndianReader to read from. It must be positioned at the start of the object table.</param>
        public ObjectList(EndianReader reader)
        {
        ...
        public void Update(EndianWriter writer)
        {
            foreach (GameObject obj in _objects)
                if (obj != null)
                    obj.Update(writer);
        ...
        private bool ProcessObject(Table table, EndianReader reader, DatumIndex index, uint size, long offset)
        {
            if (!index.IsValid)
            {
                _objects.Add(null);
